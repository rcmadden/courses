...
class Vector(object):
    ...
    def add(self, v):
        ...
        for x, y in zip(self.coordinates, v.coordinates):
            new_coordinates.append(round(x + y, 3))
    ...
    def scalar_multiply(self, scalar):
        ...
        for x in self.coordinates:
            new_coordinates.append(round(scalar * x, 3))
        return Vector(new_coordinates)
    ...
    def magnitude(self):
        coordinates_squared = []
        for x in self.coordinates:
            coordinates_squared.append(x**2)
        return round(sum(coordinates_squared) ** 0.5, 3)
...
v_diff = Vector([7.119, 8.215]).subtract(Vector([-8.223, 0.878]))
